...
class FeesRecieve extends StatefulWidget {
  const FeesRecieve({super.key});

  @override
  State<FeesRecieve> createState() => _FeesRecieveState();
}

class _FeesRecieveState extends State<FeesRecieve> {
                  ...
                  children: [
                    ...
                    Expanded(
                      child: SearchField<AdmissionList>(
                        ...
                        onSuggestionTap: (val) {
                          setState(() {
                            studentController.text = val.item!.studentName!;
                            studentId = val.item!.id.toString();
                            selectedAccountHeadLedger = val.item;
                            accountHeadBalance =
                                selectedAccountHeadLedger?.fatherName ?? '';
                            accountHeadType =
                                selectedAccountHeadLedger?.primaryContactNo ??
                                '';
                          });
                          FocusScope.of(
                            context,
  ...
  Future getStudent() async {
    var response = await ApiService.fetchData(
      "admission?licence_no=${Preference.getString(PrefKeys.licenseNo)}",
    );
    if (response["status"] == true) {
  ...
  Future postVoucher(List<File> images) async {
    ...
    final response = await ApiService.uploadMultipleFiles(
      ...
      fields: {
        ...
        'account_head': studentController.text.toString(),
  ...
  Future getVoucherId() async {
    ...
    voucherNumberController.text = response['next-fees_no'].toString();
  }
}
