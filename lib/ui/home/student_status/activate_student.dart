...
class ReactivateStudent extends StatefulWidget {
  const ReactivateStudent({super.key});

  @override
  State<ReactivateStudent> createState() => _ReactivateStudentState();
}

class _ReactivateStudentState extends State<ReactivateStudent> {
  ...
  TextEditingController joinDatePicker = TextEditingController(
  ...
  void initState() {
    ...
    getHostlers().then((_) {
      ...
      if (args != null && activateData == null) {
        activateData = args as ActivatedList;
        _fillactivateData(); // fill data only after student list is ready
      }
  ...
  void _fillactivateData() {
    studentId = activateData!.hostelerId ?? "";
    studentIdController.text = activateData!.hostelerId ?? "";
    fatherNameController.text = activateData!.fatherName ?? "";
    contactNoController.text = activateData!.contactNo ?? "";
    admissionDateController.text = activateData!.admissionDate ?? "";
    studentController.text = activateData!.hostelerName ?? "";
    remarkController.text = activateData!.remark ?? "";
    joinDatePicker.text = activateData!.joinDate ?? "";
  }

  ActivatedList? activateData;
  @override
            ...
            Padding(
              ...
              child: Column(
                children: [
                  addMasterOutside5(
                    children: [
                      TitleTextField(
                        image: Images.year,
                        hintText: "--Date--",
                        controller: joinDatePicker,
                        titileText: "",
                        onTap: () async {
                          _selectDate(context, joinDatePicker);
                        },
                      ),
                      TitleTextField(
                        image: null,
                  ...
                  Center(
                    child: DefaultButton(
                      text: activateData == null ? "Save" : 'Update',
                      hight: 40,
                      width: 150,
                      onTap: () {
                        activateData != null
  ...
  Future postReactivateStudent() async {
    final response = await ApiService.postData('reactivate', {
