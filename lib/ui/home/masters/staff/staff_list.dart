...
class _StaffListScreenState extends State<StaffListScreen> {
  ...
  List<StaffList> get _filteredData {
    List<StaffList> result = staffList; // Initialize with unfiltered data
    if (_searchQuery.isNotEmpty) {
      final query = _searchQuery.toLowerCase();

      result =
          staffList.where((item) {
                  ...
                  InkWell(
                    onTap: () async {
                      var updatedData = await Navigator.of(
                        context,
                      ).pushNamed('/create Staff', arguments: null);
              ...
              children: [
                ...
                ..._pagedData.map((item) {
                  return TableRow(
                    ...
                    children: [
                      tableBody(item.staffId ?? ''),
                      tableBody(item.staffName ?? ''),
                      tableBody(item.department ?? ''),
                      tableBody(item.designation ?? ''),
                            ...
                            children: [
                              IconButton(
                                ...
                                onPressed: () async {
                                  var updatedData = await Navigator.of(
                                    context,
                                  ).pushNamed('/create Staff', arguments: item);
                                  if (updatedData == "New Data") {
                                    getStaff().then((value) {
                                      setState(() {});
                                    });
  ...
  Future deleteStaff(int id) async {
    var response = await ApiService.deleteData("staff/$id");
    if (response["status"] == true) {
