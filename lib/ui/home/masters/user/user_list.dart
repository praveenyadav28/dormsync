...
class _UserListScreenState extends State<UserListScreen> {
  ...
  List<UserModel> get _filteredData {
    List<UserModel> result = userList; // Initialize with unfiltered data
                  ...
                  InkWell(
                    onTap: () async {
                      var updatedData = await Navigator.of(
                        context,
                      ).pushNamed('/create User', arguments: null);
              ...
              children: [
                ...
                ..._pagedData.map((item) {
                  return TableRow(
                    ...
                    children: [
                      tableBody(item.staffName ?? ''),
                      tableBody(item.userName ?? ''),
                      tableBody(item.password ?? ''),
                      TableCell(
                        child: Padding(
                            ...
                            children: [
                              IconButton(
                                ...
                                onPressed: () async {
                                  var updatedData = await Navigator.of(
                                    context,
                                  ).pushNamed('/create User', arguments: item);
                                  if (updatedData == "New Data") {
                                    getUser().then((value) {
                                      setState(() {});
                                    });
  ...
  Future getUser() async {
    var response = await ApiService.fetchData(
      "only_user?licence_no=${Preference.getString(PrefKeys.licenseNo)}&branch_id=${Preference.getint(PrefKeys.locationId)}",
    );
    if (response["status"] == true) {
      userList = userModelFromJson(response['data']);
    }
  }

  Future deleteUser(int id) async {
    var response = await ApiService.deleteData("only_user/$id");
    if (response["status"] == true) {
