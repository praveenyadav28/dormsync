...
class _LedgerListScreenState extends State<LedgerListScreen> {
  ...
  List<LedgerList> get _filteredData {
    List<LedgerList> result = ledgerList; // Initialize with unfiltered data
    if (_searchQuery.isNotEmpty) {
      final query = _searchQuery.toLowerCase();

      result =
          ledgerList.where((item) {
  ...
  List<LedgerList> get _pagedData {
                  ...
                  InkWell(
                    onTap: () async {
                      var updatedData = await Navigator.of(
                        context,
                      ).pushNamed('/create Ledger', arguments: null);
                ...
                ..._pagedData.map((item) {
                  return TableRow(
                    ...
                    children: [
                      ...
                      tableBody(item.ledgerGroup ?? ''),
                      tableBody("${item.openingBalance}/- ${item.openingType}"),
                      tableBody(item.gstNo ?? ''),
  ...
  Future deleteStaff(int id) async {
    var response = await ApiService.deleteData("ledger/$id");
    if (response["status"] == true) {
