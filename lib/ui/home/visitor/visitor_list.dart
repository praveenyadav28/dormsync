...
import 'package:dorm_sync/ui/excel/visitor_excel.dart';
...
class _VisitorListScreenState extends State<VisitorListScreen> {
  ...
  List<VisitorList> get _filteredData {
    List<VisitorList> result = visitorList; // Initialize with unfiltered data
  ...
  List<VisitorList> get _pagedData {
                  ...
                  InkWell(
                    onTap: () async {
                      var updatedData = await Navigator.of(
                        context,
                      ).pushNamed('/visitor Add', arguments: null);
                ...
                child: Row(
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: IconButton(
                        onPressed: () async {
                          await exportVisitorListToExcel(_filteredData);
                        },
                ...
                ..._pagedData.map((item) {
                  return TableRow(
                    ...
                    children: [
                      ...
                      tableBody(item.visitingDate ?? ""),
                      tableBody(item.visitorName ?? ""),
                      tableBody(item.relation ?? ""),
                      tableBody(item.contact ?? ""),
                      tableBody(item.dateOfLeave ?? ''),
                      TableCell(
                        child: Padding(
                            ...
                            children: [
                              IconButton(
                                ...
                                onPressed: () async {
                                  var updatedData = await Navigator.of(
                                    context,
                                  ).pushNamed('/visitor Add', arguments: item);
                                  if (updatedData == "New Data") {
                                    getVisitor().then((value) {
                                      setState(() {});
                                    });
                                  ...
                                  showDialog(
                                    ...
                                    builder:
                                        (dialogContext) => AlertDialog(
                                          ...
                                          content: const Text(
                                            "Are you sure you want to delete this visit history?",
                                          ),
                                          actions: [
                                            TextButton(
                                              onPressed:
  ...
  Future deleteVisitor(int id) async {
    var response = await ApiService.deleteData("visitor/$id");
    if (response["status"] == true) {
